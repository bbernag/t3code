...
describe("recent thread attention", () => {
  ...
  it("describes zero, one, and multiple attention chats accessibly", () => {
    expect(recentThreadsBubbleAccessibilityLabel(0)).toBe("Recent chats");
    expect(recentThreadsBubbleAccessibilityLabel(1)).toBe("Recent chats, 1 needs attention");
    expect(recentThreadsBubbleAccessibilityLabel(2)).toBe("Recent chats, 2 need attention");
  });
});
